...
public class Process {
		...
		if (testUnkow || testSize) { // créer une nouvelle chaine de contamination
			...
			if (chainNumber == null) { // alors on n'a pas trouvé notre contaminant dans la table de hash
				...
				if (m.getIdPays_() != chaine_.get(chainNumber).get(0).getIdPays_()) {
					...
					m.setScore_(setScoreDate(m.getDateContamined_(),chaine_.get(chainNumber).get(beforeLast).getDateContamined_()));
	...
	public long[]DataToQueue(int index) {
		...
		/**
		 * on a l'index de la chaine que l'on veut envoyer à la queue, on va maintenant mettre les données en forme pour les mettres dans la queue
		 */
